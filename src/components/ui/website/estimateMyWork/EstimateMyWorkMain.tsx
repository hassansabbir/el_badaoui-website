...
const EstimateMyWorkMain = () => {
  ...
  const [elementChecks, setElementChecks] = React.useState<Record<string, boolean>>({});
  ...
  const decRoom = (id: string) => {
    setRoomCounts((prev) => ({ ...prev, [id]: Math.max(0, (prev[id] ?? 0) - 1) }));
  };
      ...
      <Container>
        ...
        <div className="mt-10 flex items-center justify-center gap-4">
          ...
          <button
            type="button"
            onClick={next}
            className="inline-flex items-center gap-2 px-6 py-3 rounded-md bg-[#2941A1] text-white hover:bg-[#2941A1]/90"
          >
            Continue
